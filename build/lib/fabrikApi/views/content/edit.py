...
from fabrikApi.util.events import EventPeerReviewInitialized
import arrow
...
def content_modify_propose(request):
    ...
    # TODO: ensur that circular hierarchies are prevented?  (the validate_parent_type method, i think...)
    ...
    peerreview = initiate_peer_review(
        ...
        data_to_apply_on_success=jsoncontent)
    ...
    updateproposal = DBContent(
        title=jsoncontent.get('title') if jsoncontent.get('title') else '@NOCHANGE@',
        text=jsoncontent.get('text') if jsoncontent.get('text') else '@NOCHANGE@',
