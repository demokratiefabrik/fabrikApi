...
# class HasAttachedAModerationMixin(object):
#     """The HasAttachedContentTreeMixin  mixin, creates a generic relationship to
#     the contenttree table for each parent.

#     Hence, we can attach a contenttree-contenttree to any kind of objects.
#     https://docs.sqlalchemy.org/en/13/_modules/examples/generic_associations/generic_fk.html
#     """

#     def load_artificial_moderation_data(self, request):
#         """
#         Create an otherwise empty response object entailing all the artificial_moderation
#         that are linked to this instance.
#         => access them via sqlalchemy property: self.contenttrees
#         """
#         response = {}

#         if not self.artificial_moderations:
#             return(response)

#         # if not request.local_userid:
#         response['artificial_moderations'] = self.artificial_moderations
#         return(response)

#         # from fabrikApi.models import DBContentTreeProgression
#         # for artificial_moderation in self.artificial_moderations:
#         #     artificial_moderation.progression = get_or_create_progression(
#         #         request, DBAModerationProgression,
#         #         auto_create=False, contenttree_id=contenttree.id,
