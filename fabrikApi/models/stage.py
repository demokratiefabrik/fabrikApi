...
from sqlalchemy import JSON, Index, Integer, Unicode, UnicodeText, Boolean,\
    Column, ForeignKey
...
class DBStageProgression(BaseProgressionObject, Base):
    ...
    def setFocusedContent(self, content):
        ...
        assert not self.focused_content_id
        self.focused_content_id = content.id
        # if not self.focused_content_id:
        # else:
        #     # ignore this request. It should not be possible to change the focused content => Manipulation
        #     # TODO raise Error? (why this happes frequently?)
        #     pass
